 ...
 * 
 * Note: Filters could not easily unregistered. Try setting @link {@link #cacheFilters} to <code>false</code>.
 * 
 * 
 * @author gk
 * @version
 *
 */
public class JacksonMapperService extends AbstractLogEnabled
    implements JsonService, Initializable, Configurable
{
    ...
    public boolean cacheFilters = true; // more efficient if not using multiple serialization in one thread
    ...
    private <T> void setCustomIntrospectorWithExternalFilterId(Class<T> externalFilterId)
    {
        if (primary instanceof CustomIntrospector) {
            if (externalFilterId != null) {
                ...
                getLogger().debug( "added class from filters " +externalFilterId.getName() );
            }
        }
    }
            ...
            while (enumFeatureKey.hasMoreElements()) {
                ...
                if(featureKey != null && featureValue != null) {
                    try
                    {
                        ...
                        getLogger().info("initializing mapper feature: "+ featureParts[featureParts.length-1] + " with "+ featureValue);
                        feature = Feature.valueOf( featureParts[featureParts.length-1] );
                        mapper.configure(feature, featureValue );
                        
                        assert mapper.getSerializationConfig().isEnabled( feature ) == featureValue;
                    }
                    catch (Exception e )
                    {
